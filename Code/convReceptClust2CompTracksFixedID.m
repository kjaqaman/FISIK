function compTracks = convReceptClust2CompTracksFixedID(clust2recept,recept2clust,...
    receptorTraj,receptorIntensity)
%CONVRECEPTCLUST2COMPTRACKFIXEDID generates compound tracks out of receptor clustering information over time
%
...
%
% Modified by Fnu Bilal, Feb 2022, for fixed ID
%
...
[~,sizeClustMax,numFrames] = size(clust2recept);

%get number of receptors system dimensionality
[numReceptors,dimension,~] = size(receptorTraj);
...
numClustFrame1 = length(clust2receptFrame1(:,1));
...
%             clustersNextFrame =recept2clust(clusterMembers,iFrame+1);
%             clustersNextFrame =intersect(clustersNextFrame, clustersNextFrame);
        ...
        % Modification, Bilal, 2023/02/16, why need to change the size here?
...
%             clustMapTmp = intersect(clustMapTmp, clustMapTmp);
        ...
        end %(for iClustBig = clustersBigNext')
